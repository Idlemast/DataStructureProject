...
int filter_ids_with_rule(StationNode* r, char* toks[], int n_toks, int* out, int cap, int min_power, int min_slots);/* O(n) */
...
int si_export_csv(StationNode* r, const char* path);    /* O(n) */
int si_export_csv_rec(StationNode* r, FILE* f);         /* O(n) */
int si_top_k_by_score(StationNode* r, int k, int* out_ids, int alpha, int beta, int gamma); /* O(n log k) */
int si_range_ids(StationNode* r, int lo, int hi, int* out, int cap); /* O(k + log n) */
int si_count_ge_power(StationNode* r, int P); /* O(n) */
